...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAll() {
      ...
      // Education
      renderSection('#education', resumeData.education.map((ed, i) => `
        <div class="edu-item">
          <strong>${ed.degree}</strong> @ ${ed.school} | ${ed.period}${ed.gpa ? ' | GPA: ' + ed.gpa : ''}
          <button class="edit-edu" data-index="${i}">Edit</button>
        </div>
      `).join('') + '<button id="addEdu">Add Education</button>');
  
      // Skills
      renderSection('.skills', resumeData.skills.map((sk, i) => `
        <div class="skill-row">
          <span>${sk.name}</span>
          <div class="skill-bar" data-level="${sk.level}"></div>
          <button class="edit-skill" data-index="${i}">Edit</button>
        </div>
      `).join('') + '<button id="addSkill">Add Skill</button>');
  
      // Languages
      renderSection('#languages', resumeData.languages.map((lang, i) => `
        <div class="lang-item">
          ${lang} <button class="edit-lang" data-index="${i}">Edit</button>
        </div>
      `).join('') + '<button id="addLang">Add Language</button>');
      ...
      // Work Experience
      renderSection('#work-experience', resumeData.workExperience.map((we, i) => `
        <div class="we-item">
          <strong>${we.role}</strong> @ ${we.company} | ${we.period}
          <button class="edit-we" data-index="${i}">Edit</button>
        </div>
      `).join('') + '<button id="addWE">Add Experience</button>');
  
      // References
      renderSection('#reference', resumeData.references.map((ref, i) => `
        <div class="ref-item">
          ${ref.name}, ${ref.title}<br>
          ${ref.phone} | ${ref.email}
          <button class="edit-ref" data-index="${i}">Edit</button>
        </div>
      `).join('') + '<button id="addRef">Add Reference</button>');
